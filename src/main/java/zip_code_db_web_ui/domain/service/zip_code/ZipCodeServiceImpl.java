...
public class ZipCodeServiceImpl implements ZipCodeService {
  ...
  public List<ZipCode> find(ZipCode zipcode) {
    ...
    if (isNotEmpty.test(zipcode.getZipCode())) {
      ...
      recordset =
          repository.findByPrefectureAndCityLike(zipcode.getPrefecture(), zipcode.getCity());
    } else if (isNotEmpty.test(zipcode.getPrefecture()) && !isNotEmpty.test(zipcode.getCity())
        && isNotEmpty.test(zipcode.getArea())) {
      // 都道府県名と町域名が指定されている場合
      recordset =
          repository.findByPrefectureAndAreaLike(zipcode.getPrefecture(), zipcode.getArea());
    } else if (isNotEmpty.test(zipcode.getPrefecture()) && isNotEmpty.test(zipcode.getCity())
        && isNotEmpty.test(zipcode.getArea())) {
      // 都道府県名・市区郡名・町域名が指定されている場合
      recordset = repository.findByPrefectureAndCityLikeAndAreaLike(zipcode.getPrefecture(),
          zipcode.getCity(), zipcode.getArea());
    } else if (!isNotEmpty.test(zipcode.getPrefecture()) && isNotEmpty.test(zipcode.getCity())
        && isNotEmpty.test(zipcode.getArea())) {
      // 市区郡名と町域名が指定されている場合
